...
module U =  GlobalPollenProject.App.UseCases
module R = ReadModels
...
let inline bindJson< ^T> (ctx:HttpContext) =
    ...
    use reader = new System.IO.StreamReader(body, true)
...
let inline api< ^T, ^R> (action:^T->Result< ^R,ServiceError>) next ctx =
    bindJson< ^T> ctx
    |> bind action
    |> apiResult next ctx
...
let routes : HttpHandler =
    choose [
        ...
        POST >=> route  "/User/Register"        >=> api (getCurrentUser |> U.User.register)

        // Curation
        GET  >=> route  "/Curate/Pending"       >=> api U.Curation.listPending
        POST >=> route  "/Cutrate/Assign"       >=> api U.User.grantCuration
        POST >=> route  "/Curate/Decide"        >=> api (getCurrentUser |> U.Curation.issueDecision)
        ...
        POST >=> route  "/Unknown/Submit"       >=> api (U.UnknownGrains.submitUnknownGrain getCurrentUser)
        POST >=> route  "/Unknown/Identify"     >=> api (U.UnknownGrains.identifyUnknownGrain getCurrentUser)

        // User's equipment
        GET >=> route   "/User/Microscope"      >=> api U.Calibrations.getMyCalibrations
        POST >=> route  "/User/Microscope/Setup"        >=> api (getCurrentUser |> U.Calibrations.setupMicroscope)
        POST >=> route  "/User/Microscope/Calibrate"    >=> api U.Calibrations.calibrateMagnification
        ...
        POST >=> route  "/Digitise/Slide/Add"       >=> api U.Digitise.addSlideRecord
        POST >=> route  "/Digitise/Slide/Void"      >=> api U.Digitise.voidSlide
        POST >=> route  "/Digitise/Slide/AddImage"  >=> api U.Digitise.uploadSlideImage

        // Administration
        POST >=> route "/Admin/RebuildReadModel"    >=> api U.Admin.rebuildReadModel
        POST >=> route "/Admin/Users"               >=> api U.Admin.listUsers
    ]
